 ...
 * DrawInfo::rtBase (a render target keyed by guest address), and endFrame reads
 * back the target at the scanout address to present it (or dump it, headless).
 */
...
// the render target at `scanoutBase` (the flip buffer) and presents/dumps it.
void beginFrame();
