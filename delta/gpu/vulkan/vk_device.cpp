...
void imageBarrier(VkCommandBuffer c, VkImage img, VkImageLayout from,
                   ...
                   uint32_t layers, uint32_t mip_levels) {
...
bool createDevice() {
  ...
  for (VkPhysicalDevice d : devs) {
    ...
    if (score > best) { best = score; g_dev.phys = d; }
  }
  if (g_dev.phys == VK_NULL_HANDLE) { std::fprintf(stderr, "[gpuvk] no gfx device\n"); return false; }
  ...
  for (uint32_t i = 0; i < qn; i++)
    if (qprops[i].queueFlags & VK_QUEUE_GRAPHICS_BIT) { g_dev.qfam = i; found = true; break; }
  ...
  const char *devExts[1] = {};
  {
    ...
    vkEnumerateDeviceExtensionProperties(g_dev.phys, nullptr, &en, eprops.data());
  ...
  g_dev.samplerAnisotropy = wantFeat.samplerAnisotropy;
  g_dev.samplerMirrorClamp = f12.samplerMirrorClampToEdge;
  g_dev.geometryShader = wantFeat.geometryShader;
  g_dev.storageImageWriteWithoutFormat =
  ...
  p_vkCmdBeginRendering = (PFN_vkCmdBeginRenderingKHR)vkGetDeviceProcAddr(g_dev.device, "vkCmdBeginRendering");
  p_vkCmdEndRendering = (PFN_vkCmdEndRenderingKHR)vkGetDeviceProcAddr(g_dev.device, "vkCmdEndRendering");
  if (!p_vkCmdBeginRendering) {
    p_vkCmdBeginRendering = (PFN_vkCmdBeginRenderingKHR)vkGetDeviceProcAddr(g_dev.device, "vkCmdBeginRenderingKHR");
    p_vkCmdEndRendering = (PFN_vkCmdEndRenderingKHR)vkGetDeviceProcAddr(g_dev.device, "vkCmdEndRenderingKHR");
  }
  ...
  VKOK(vkCreateFence(g_dev.device, &fc, nullptr, &g_dev.fence));  // aux submits only
  if (!createFrameSlots()) return false;

  VkPhysicalDeviceProperties props;
  vkGetPhysicalDeviceProperties(g_dev.phys, &props);
  g_dev.maxCsResources = std::min(
      {gcn::kMaxCsResources, props.limits.maxPerStageDescriptorStorageBuffers,
       props.limits.maxDescriptorSetStorageBuffers});
  g_dev.maxStorageBufferRange = props.limits.maxStorageBufferRange;
  std::fprintf(stderr, "[gpuvk] device: %s\n", props.deviceName);
  if (!createUploadRings(props)) return false;
  return true;
}
...
bool available() { return g_dev.ready; }
