...
- **Present is already neutral.** `gfx/` owns its own `VkInstance`/`VkDevice`
  and the gpu-to-window handoff is a CPU pixel buffer (`gfx::present`). There is
  no swapchain on the render device, so swapchain interop is not part of this
  work. `gfx.h` itself names no API type; only its implementation is Vulkan.
- **Shaders are not neutral.** The recompiler emits SPIR-V with Vulkan
  descriptor-set decorations baked in (set 0 textures, set 1 dynamic UBOs, set 2
  raw buffers, set 3 LDS scratch), and `gcn_translate.h` reasons about
  `maxDescriptorSetStorageBuffersDynamic`. This is the one genuinely new
  subsystem D3D12 needs.
...
### The RHI vocabulary
Derive it, do not invent it. The 82 calls collapse to roughly fifty operations:
...
1. **Resource state, not image layout.** Today `RTarget` tracks `VkImageLayout`
   and `submitted_layout` by hand. Replace with a neutral `rhi::ResourceState`
   (RenderTarget, ShaderRead, DepthWrite, DepthRead, CopySrc, CopyDst, Common).
   It maps to `VkImageLayout` + access mask on one side and
   `D3D12_RESOURCE_STATES` on the other, and the backend owns barrier batching.
2. **Render pass with load/store ops.** Vulkan's dynamic rendering needs them;
   D3D12 can honour them through render passes or ignore them. The lazy
   `clear_pending` mechanism in `vk_render_target` maps onto `loadOp` unchanged.
3. **Bind groups with dynamic offsets.** Set 1 is dynamic UBOs, which D3D12 has
   no equivalent for. Hide it: `BindGroup` carries an offset array, the Vulkan
   backend uses dynamic descriptors, the D3D12 backend uses root CBVs or
   per-draw descriptor copies out of a ring heap.
4. **Host import is a capability, not an abstraction.** `host_import_available`
   (`VK_EXT_external_memory_host`) aliases guest pages directly and is why the
   compute path avoids copying whole allocations. D3D12 cannot do this; the
   fallback is the existing staging copy, and the cost is real.
...
1. **A null recording backend** (`rhi/null`). Implements the full RHI, hands out
   fake handles, records every call into a typed command log, serves readback
   deterministically. `Draw(DrawInfo)` becomes a pure function from guest state
   to a command log.
2. **Golden command-log tests.** `DELTA_GPU_CAPTURE` already dumps a whole guest
   frame as JSONL. Reuse that as test *input*: replay a captured frame from a
   real title through the null backend and assert on the log. This is the
...
4. **A backend conformance suite.** One binary runs identical scenarios against
   every compiled-in backend and compares readback pixels. Needs a GPU, so it is
   a developer target rather than CI, and it is the only thing that keeps D3D12
   honest against Vulkan.
5. **Remove the globals.** `Renderer&` is already threaded through the `rhi`
   entry points; finish the job so state comes from the argument rather than
...
**3. Carve the RHI.** Define `rhi/device.h` and `rhi/command_list.h` from the 82
...
## Alternatives
- **Write a D3D12 backend under today's seam.** Rejected: it duplicates ~17k
...
- **Translate in the command processors (hand the backend an already-neutral
  draw).** Rejected: it puts graphics-API policy in the PM4/AGC walks, which the
  current design deliberately keeps out of them, and it would be a much larger
  change to files that are under active investigation.
- **Emit DXIL directly from the recompiler.** Rejected for now: DXIL is LLVM 3.7
  bitcode plus signing, and a second emitter is a larger project than the whole
  D3D12 backend. Cross-compilation reuses a translator that is already correct
  for many titles. Revisit if compile time becomes the bottleneck (see
  `shader-hitch-causes`).
- **Adopt an existing RHI (Dawn, wgpu-native, bgfx, nvrhi).** Rejected: guest
...
- **Skip phases 0 to 5 and start with D3D12.** Rejected: without the null
  backend, the only way to know the D3D12 backend is correct is to boot a title
  and look at it, which is how the current renderer became untestable.
...
## Acceptance
- `gpu_layering` enforces the new rules and `gpu/render` contains no `Vk*` or
  `ID3D12*` identifier.
- The null backend replays a captured Isaac frame and the command log matches a
  checked-in golden.
- A `verify.sh` baseline taken before phase 1 matches after phase 4.
- The conformance suite renders identical readback for Vulkan and D3D12 on the
  quad, recompiled-draw, RT-as-texture, MRT, depth and compute scenarios.
- The existing `ps4delta` target builds and Isaac still renders at parity FPS on
  Vulkan.
